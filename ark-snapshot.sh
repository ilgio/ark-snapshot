...
if [ ! -f ../ark-node/app.js ]; then
  echo "Error: No ark installation detected. Exiting."
  exit 1
fi

if [ "\$USER" == "root" ]; then
  echo "Error: ARK should not be run be as root. Exiting."
  exit 1
fi
...
if [ ! -f "snapshot/counter.json" ]; then
  mkdir -p snapshot
  sudo chmod a+x ark-snapshot.sh
...
create_snapshot() {
  ...
  echo "--------------------------------------------------"
  echo "..."
  sudo su postgres -c "pg_dump -Ft $DB_NAME > $SNAPSHOT_DIRECTORY'ark_db$NOW.snapshot.tar'"
